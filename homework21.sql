...
SELECT _jobs.job_title, CONCAT(_employees.first_name, ' ', _employees.last_name) AS "full_name",
       (_jobs.max_salary - _employees.salary) AS "diff_salary"
FROM _jobs
JOIN _employees ON _jobs.job_id = _employees.job_id;

SELECT _jobs.job_title, ROUND(AVG(_employees.salary), 2) AS "average_salary"
FROM _jobs
JOIN _employees ON _jobs.job_id = _employees.job_id
